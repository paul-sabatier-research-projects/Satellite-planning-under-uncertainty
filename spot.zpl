...
# Fonction d'objectif avec ajustement des gains (Changer Pinf[i] avec Psup[i] pour un decideur optimiste)
maximize valeur : sum <i> in images :
   sum <j> in instruments: 
      (1/type[i]) * selection[i] * PA[i] * (1 - (Pinf[i])) * assignedTo[i,j] * (1 - Pfail[j]);
...
subto correspondancemono:
   forall <ima> in images with type[ima]==1  : 
      assignedTo[ima,1] + assignedTo[ima,2] + assignedTo[ima,3] == selection[ima] ;

subto correspondancestereo1:
   forall <ima> in images     with type[ima]==2  :
         assignedTo[ima,1]== selection[ima] ; 

subto correspondancestereo2:
   forall <ima> in images     with type[ima]==2  :
      assignedTo[ima,2]== 0 ;
      

subto correspondancestereo3:
   forall <ima> in images     with type[ima]==2  :
